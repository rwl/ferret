...
class IndexReader {
  ...
  factory IndexReader(Ferret ferret, Directory dir) {
    ...
    if (dir is List) {
      ...
      h = ferret.callFunc('frt_ir_open', [dir.handle]);
  ...
  void commit() => _ferret.callFunc('frjs_ir_commit', [handle]);
  ...
  void delete(int doc_id) =>
      _ferret.callFunc('frt_ir_delete_doc', [handle, doc_id]);

  /// Returns `true` if the document at [doc_id] has been deleted.
  bool deleted(int doc_id) =>
      _ferret.callFunc('frjs_ir_is_deleted', [handle]) != 0;
  ...
  num max_doc() => _ferret.callFunc('frjs_ir_max_doc', [handle]);
  ...
  /// from the index.
  int num_docs() => _ferret.callFunc('frjs_ir_num_docs', [handle]);
  ...
  void undelete_all() => _ferret.callFunc('frt_ir_undelete_all', [handle]);
  ...
  bool latest() => _ferret.callFunc('frt_ir_is_latest', [handle]) != 0;
  ...
  LazyDoc get_document(int id) {
    ...
    int p_doc = _ferret.callFunc('frjs_ir_get_lazy_doc', [handle, pos]);
  ...
  TermVector term_vector(int doc_id, String field) {
    ...
    int p_tv =
        _ferret.callFunc('frjs_ir_term_vector', [handle, doc_id, p_field]);
  ...
  Map<String, TermVector> term_vectors(int doc_id) {
    int p_tvs = _ferret.callFunc('frjs_ir_term_vectors', [handle, doc_id]);
    int h_size = _ferret.callFunc('frjs_hash_get_size', [p_tvs]);
    var m = new Map<String, TermVector>();
    for (int i = 0; i < h_size; i++) {
      int p_key = _ferret.callFunc('frjs_hash_get_key', [p_tvs, i]);
      int p_val = _ferret.callFunc('frjs_hash_get_value', [p_tvs, i]);
      var key = _ferret.stringify(p_key, false);
      m[key] = new TermVector._handle(_ferret, p_val);
    }
    _ferret.callFunc('h_destroy', [p_tvs]);
    return m;
  }
  ...
  TermDocEnum term_docs() {
    int p_tde = _ferret.callFunc('frjs_ir_term_docs', [handle]);
  ...
  TermDocEnum term_positions() {
    int p_tde = _ferret.callFunc('frjs_ir_term_positions', [handle]);
    return new TermDocEnum._handle(_ferret, p_tde, _field_num_map);
  }
  ...
  TermDocEnum term_docs_for(String field, String term) {
    ...
    int p_tde =
        _ferret.callFunc('frt_ir_term_docs_for', [handle, symbol, p_term]);
    _ferret.free(p_term);
  ...
  TermDocEnum term_positions_for(String field, String term) {
    ...
    int p_term = _ferret.heapString(term);
    int p_tde =
        _ferret.callFunc('frt_ir_term_positions_for', [handle, symbol, p_term]);
    _ferret.free(p_term);
    return new TermDocEnum._handle(_ferret, p_tde, _field_num_map);
  }
  ...
  int doc_freq(String field, String term) {
    ...
    int p_term = _ferret.heapString(term);
    int freq = _ferret.callFunc('frt_ir_doc_freq', [handle, symbol, p_term]);
    _ferret.free(p_term);
    return freq;
  }
  ...
  TermEnum terms(String field) {
    ...
    int p_te = _ferret.callFunc('frt_ir_terms', [handle, symbol]);
  ...
  TermEnum terms_from(String field, String term) {
    ...
    int p_term = _ferret.heapString(term);
    int p_te = _ferret.callFunc('frt_ir_terms_from', [handle, symbol, p_term]);
    _ferret.free(p_term);
  ...
  int term_count(String field) {
    int p_field = _ferret.heapString(field);
    int cnt = _ferret.callFunc('frjs_ir_term_count', [handle, p_field]);
    _ferret.free(p_field);
    return cnt;
  }
  ...
  FieldInfos get field_infos {
    int p_fis = _ferret.callFunc('frjs_ir_field_infos', [handle]);
  ...
  int get version => _ferret.callFunc('frjs_ir_version', [handle]);
}
